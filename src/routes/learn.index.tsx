  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8 pb-24 md:pb-8">
      <header className="mb-8">
        ...
        <h1 className="mt-1 font-serif text-3xl font-bold text-foreground">Learn chess</h1>
        <p className="mt-2 text-sm text-muted-foreground max-w-xl">
          Bite-sized interactive lessons. Read the tip, play the move, move on. No account required
          to try — sign in to save your progress.
        </p>
      </header>

      {(
        [
          ["basics", "The basics", "How the pieces move, checkmate and opening principles."],
          [
            "middlegame",
            "Middlegame",
            "Tactics and attacking ideas, practised on real positions from games.",
          ],
        ] as const
      ).map(([topic, heading, blurb]) => (
        <section key={topic} className="mb-8">
          ...
          <ul className="grid gap-3 sm:grid-cols-2">
            {TUTORIALS.filter((t) => (t.topic ?? "basics") === topic).map((t) => {
              const p = progressMap.get(t.id);
              const done = p?.completed ?? false;
              const stepIdx = p?.step_index ?? 0;
              const pct = done ? 100 : Math.round((stepIdx / t.steps.length) * 100);
              return (
                <li key={t.id}>
                  <Link
                    to="/learn/$tutorialId"

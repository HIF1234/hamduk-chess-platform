...
/** Verifies a request's `Authorization: Bearer` Supabase token (including the two-factor
 *  rule) and returns a Supabase client acting as that user. Used by server functions through
 *  requireSupabaseAuth and directly by the mobile app API. */
export async function authenticateRequest(request: Request | undefined) {
...
export const requireSupabaseAuth = createMiddleware({ type: "function" }).server(
  async ({ next }) => {
    const context = await authenticateRequest(getRequest());
    return next({ context });
  },
);

-- Puzzle attempt log (spec: puzzle_attempts) and the free-tier daily limit,
-- enforced inside the RPC so no client can bypass it.
...
CREATE INDEX puzzle_attempts_user_created_idx ON public.puzzle_attempts (user_id, created_at DESC);

GRANT SELECT ON public.puzzle_attempts TO authenticated;
GRANT ALL ON public.puzzle_attempts TO service_role;
ALTER TABLE public.puzzle_attempts ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users read their own puzzle attempts"
  ON public.puzzle_attempts FOR SELECT TO authenticated
  USING (user_id = auth.uid());

CREATE OR REPLACE FUNCTION public.free_daily_puzzle_limit()
RETURNS integer LANGUAGE sql IMMUTABLE AS $$ SELECT 20 $$;

CREATE OR REPLACE FUNCTION public.puzzles_attempted_today(p_user uuid)
RETURNS integer
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT count(DISTINCT puzzle_id)::int FROM public.puzzle_attempts
  WHERE user_id = p_user
    AND created_at >= date_trunc('day', now() AT TIME ZONE 'UTC') AT TIME ZONE 'UTC'
$$;
REVOKE EXECUTE ON FUNCTION public.puzzles_attempted_today(uuid) FROM PUBLIC, anon, authenticated;
